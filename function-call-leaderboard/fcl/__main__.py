...
import typer
from fcl._llm_response_generation import main as generation_main
from fcl.constants.category_mapping import TEST_COLLECTION_MAPPING
...
from fcl.eval_checker.eval_runner import main as evaluation_main
...
def generate(
    ...
    result_dir: str = typer.Option(
        RESULT_PATH,
        "--result-dir",
        help="Path to the folder where output files will be stored; Path should be relative to the `function-call-leaderboard` root folder",
    ),
...
def results(
    result_dir: str = typer.Option(
        None,
        "--result-dir",
        help="Relative path to the model response folder, if different from the default; Path should be relative to the `function-call-leaderboard` root folder",
    ),
):
    """
...
def evaluate(
    ...
    score_dir: str = typer.Option(
        None,
        "--score-dir",
        help="Relative path to the evaluation score folder, if different from the default; Path should be relative to the `function-call-leaderboard` root folder",
    ),
):
    """
